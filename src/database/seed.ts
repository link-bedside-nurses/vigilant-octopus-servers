...
async function seedAppointments( patients: any[], nurses: any[] ) {
	...
	for ( let i = 0; i < 200; i++ ) {
		...
		if ( nurse ) {
			...
			appointment.assignedBy = faker.helpers.arrayElement( [...Array( 5 )].map( () => faker.database.mongodbObjectId() ) ); // Random admin ID
		...
		if ( status === APPOINTMENT_STATUSES.CANCELLED ) {
			...
			appointment.cancelledBy = faker.helpers.arrayElement( [...Array( 5 )].map( () => faker.database.mongodbObjectId() ) ); // Random admin ID
		}
