...
async function processAccountDeletions() {
	try {
		...
		console.info(
			`Found ${nursesToDelete.length} nurses and ${patientsToDelete.length} patients to delete`
		);

		// Delete each nurse
		for (const nurse of nursesToDelete) {
			console.info(`Deleting nurse: ${nurse._id}`);
			await db.nurses.findByIdAndDelete(nurse._id.toString());
		}

		// Delete each patient
		for (const patient of patientsToDelete) {
			console.info(`Deleting patient: ${patient._id}`);
			await db.patients.findByIdAndDelete(patient._id.toString());
		}

		console.info(
			`Successfully deleted ${nursesToDelete.length} nurses and ${patientsToDelete.length} patients`
		);
	} catch (error) {
