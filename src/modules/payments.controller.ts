...
router.post( '/patient/:id/initiate', authenticate, async ( req: Request, _res: Response, next: NextFunction ) => {
    try {
        ...
        const patient = await PatientRepo.getPatientById( req.params.id );
        if ( !patient ) {
...
router.get( '/:id/status', authenticate, async ( req: Request, _res: Response, next: NextFunction ) => {
    try {
        const payment = await db.payments.findById( req.params.id );
        if ( !payment ) {
...
router.get( '/nurses/:id/earnings', authenticate, async ( req: Request, _res: Response, next: NextFunction ) => {
    try {
...
router.get( '/patients/:id/payments', authenticate, async ( req: Request, _res: Response, next: NextFunction ) => {
    try {
