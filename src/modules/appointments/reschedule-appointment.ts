...
export default function rescheduleAppointment() {
    return async function ( request: HTTPRequest<object, RescheduleAppointmentDto, object> ) {
        ...
        const appointment = await AppointmentRepo.getAppointmentById( result.data.appointmentId );
        if ( !appointment ) {
            return response( StatusCodes.NOT_FOUND, null, 'Appointment not found' );
        }

        const updatedAppointment = await AppointmentRepo.rescheduleAppointment( result.data.appointmentId, result.data.date );
